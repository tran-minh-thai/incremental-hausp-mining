#!/usr/bin/env python3
"""Which cells the manuscript prints are not what the current code measures.

After the campaign on the measurement machine every printed cell should be current; this is the
check that says so, cell by cell, and names the reason when one is not. The commands of a
campaign are generated by analysis/campaign_plan.py from PROTOCOL below.
...
2026-09-25 -- one control, so this is a working assumption the next campaign re-tests).
    ...
    python3 analysis/stale_cells.py        # report; exit 1 if a printed cell is stale or a trial was written twice
"""
...
def datasets_of(kind: str, exp: int, cfg: dict) -> list[str]:
    ...
    return [r["csv_name"] for r in cfg[exp]["runs"] if r["csv_name"] != "example"]
